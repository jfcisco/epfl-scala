...
trait Solver extends GameDef:
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] =
    (for (blockAfterMove, move) <- b.legalNeighbors yield (blockAfterMove, move :: history)).to(LazyList)
  ...
  def newNeighborsOnly(neighbors: LazyList[(Block, List[Move])],
    ...
    for (block, history) <- neighbors if !explored.contains(block) yield (block, history)
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    val frontier =
      for
        (block, moves) <- initial
        neighbors = neighborsWithHistory(block, moves) // expand search space with legal moves from this state
        newNeighbor <- newNeighborsOnly(neighbors, explored) // eliminate states previously found
      yield newNeighbor

    // Concatenate lists
    initial #::: from(frontier, explored ++ frontier.map((block, _) => block))

  /**
   * The lazy list of all paths that begin at the starting block.
   */
  lazy val pathsFromStart: LazyList[(Block, List[Move])] =
    val initial = LazyList((startBlock, Nil))
    from(initial, Set())
  ...
  lazy val pathsToGoal: LazyList[(Block, List[Move])] =
    import Move.*
    def invertMove(m: Move) = m match
      case Up => Down
      case Down => Up
      case Left => Right
      case Right => Left

    val endBlock = Block(goal, goal)
    val endState = LazyList((endBlock, Nil))
    from(endState, Set()).map(
      (block, moves) => (block, moves.map(invertMove))
    )
  ...
  lazy val solution: List[Move] =
    /* Express solutions as a bidirectional search with pathsFromStart serving as the forward search
      and pathsToGoal as the backward search */
    val solutions =
      for
        (blockFromStart, movesFromStart) <- pathsFromStart
        (blockFromGoal, movesToGoal) <- pathsToGoal
        if blockFromStart == blockFromGoal
      yield movesFromStart.reverse ++ movesToGoal
    solutions.head // First element should be the shortest path
