...
trait EncoderInstances:
  ...
  given Encoder[String] = Encoder.fromFunction(Json.Str(_))

  /** An encoder for `Boolean` values */
  given Encoder[Boolean] = Encoder.fromFunction(Json.Bool(_))
...
trait DecoderInstances:
  ...
  given Decoder[Int] =
    Decoder.fromPartialFunction { case Json.Num(num) if num.isValidInt  => num.toIntExact }
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] = 
    Decoder.fromFunction {
      case Json.Arr(items) =>
        val decodedItems = for item <- items yield decoder.decode(item)
        if decodedItems.contains(None) then None
        else Some(decodedItems.map(_.get))
      case _ => None
    }
...
trait ContactsCodecs:
  ...
  given Decoder[Contacts] =
    Decoder.field[List[Person]]("people").transform(Contacts(_))
